...
macro_rules! constantify {
	($input: expr) => {{
		($input).bytes().map(|x| x as i32).collect()
	}};
}
...
impl Parser {
	...
	fn pop_operation(
		pos: &Option<Position>,
		output_stack: &mut Vec<Expression>,
		operations_stack: &mut Vec<OperationType>,
	) -> Option<()> {
		let rhs = output_stack.pop();
		if rhs.is_none() {
			match pos {
				| Some(pos) => {
					parsing_error!(pos, "Not enough operands")
				}
				| None => {
					parsing_error!("Not enough operands")
				}
			}
		}
		let rhs = rhs.unwrap();
		...
		match operations_stack.pop() {
			| None => match pos {
				| Some(pos) => {
					parsing_error!(pos, "Not enough operators")
				}
				| None => {
					parsing_error!("Not enough operators")
				}
			},
			| Some(OperationType::UnaryOp(op)) => {
				output_stack.push(Expression::UnaryOp(op, Box::new(rhs)));
				return Some(());
			}
			| Some(OperationType::BinaryOp(op)) => {
				...
				output_stack.push(Expression::BinaryOp(op, Box::new(lhs), Box::new(rhs)));
				return Some(());
			}
		}
	}
			...
			match tok {
							...
							match self.parse_block(true, true, Parser::parse_expression) {
								...
								| Some(args) => {
									if args.len() == 1 {
										...
										output_stack.push(Expression::FunctionCall(s, args));
									}
								}
							}
						}
				...
				| Token::Colon => {
					...
					match &output_stack[0] {
						...
						| Expression::Identifier(vn) => match self.parse_expression() {
							| None => {
								return None;
							}
							| Some(e) => {
						...
						| Some((Token::LParan, _)) => {
							...
							match self.parse_constants() {
								...
								| Some(vals) => {
									...
									if self
										.flatten_variable_initial_value(
											&mut array_vals,
											&vals,
											&pos,
										)
				...
				| Token::Keyword("NOT") => {
					handle_op!(OperationType::UnaryOp(UnaryOperation::Not))
				}

				| Token::Keyword("PLUS") => {
					handle_op!(OperationType::BinaryOp(BinaryOperation::AddWithCarry))
				}
				| Token::Plus => {
					handle_op!(OperationType::BinaryOp(BinaryOperation::Add));
				}
				| Token::Keyword("MINUS") => {
					handle_op!(OperationType::BinaryOp(BinaryOperation::SubstractWithCarry))
				}
				| Token::Minus => {
				...
				| Token::Star => {
					handle_op!(OperationType::BinaryOp(BinaryOperation::Multiply))
				}
				| Token::Slash => {
					handle_op!(OperationType::BinaryOp(BinaryOperation::Division))
				}
				| Token::Keyword("MOD") => {
					handle_op!(OperationType::BinaryOp(BinaryOperation::Modulo))
				}

				| Token::Keyword("AND") => {
					handle_op!(OperationType::BinaryOp(BinaryOperation::And))
				}
				| Token::Keyword("OR") => {
					handle_op!(OperationType::BinaryOp(BinaryOperation::Or))
				}
				| Token::Keyword("XOR") => {
					handle_op!(OperationType::BinaryOp(BinaryOperation::Xor))
				}

				| Token::Equal => {
					handle_op!(OperationType::BinaryOp(BinaryOperation::Equal))
				}
				| Token::NotEqual => {
					handle_op!(OperationType::BinaryOp(BinaryOperation::NotEqual))
				}
				| Token::Greater => {
					handle_op!(OperationType::BinaryOp(BinaryOperation::Greater))
				}
				| Token::GreaterEqual => {
					handle_op!(OperationType::BinaryOp(BinaryOperation::GreaterOrEqual))
				}
				| Token::Less => {
					handle_op!(OperationType::BinaryOp(BinaryOperation::Less))
				}
				| Token::LessEqual => {
					handle_op!(OperationType::BinaryOp(BinaryOperation::LessOrEqual))
				}
	...
	*/
	...
	fn parse_paranthesis_less_block<T>(
		&mut self,
		parse: fn(&mut Self) -> Option<T>,
	) -> Option<Vec<T>> {
		let mut output = vec![];
	...
	fn parse_identifier_block(&mut self, show_error: bool) -> Option<Vec<String>> {
		self.parse_block(show_error, false, |parser| -> Option<String> {
		...
		match tok {
			| Token::Keyword("END") => {
				...
				match self.lexer.next() {
					| Some((Token::SemiColon, _)) => Some(Statement::EndOfStatement(None)),
					| Some((Token::Identifier(s), _)) => {
			...
			| Token::Keyword("GOTO") | Token::Keyword("GO") => {
				self.lexer.next();
				...
				match self.lexer.next() {
					...
					| Some((_, pos)) => {
						parsing_error!(pos, "Invalid token for GOTO")
					}
					| None => {
						parsing_error!("Invalid token for GOTO")
					}
				}
			}
			| Token::Keyword("IF") => {
				...
				match self.parse_statement(None) {
					...
					| Some(then_blk) => match self.lexer.peek() {
						| Some((Token::Keyword("ELSE"), _)) => {
							self.lexer.next();

							match self.parse_statement(None) {
								| None => None,
								| Some(else_blk) => Some(Statement::IfElse(
									condition,
									Box::new(then_blk),
									Box::new(else_blk),
								)),
				...
				match tok.clone() {
					...
					| Token::Keyword("WHILE") => {
						...
						match self.parse_statement_block(None) {
							| None => None,
							| Some(blk) => Some(Statement::While(condition, Box::new(blk))),
						}
					}
					/* Do case block */
					| Token::Keyword("CASE") => {
						...
						match self.parse_statement_block(None) {
							...
							| Some(stmt) => Some(Statement::DoCase(condition, vec![stmt])),
						}
					}
					/* For loop */
					| Token::Identifier(var) => {
						...
						match (origin, destination) {
							| (Some(src), Some(dst)) => match self.parse_statement_block(None) {
								| Some(blk) => {
									return Some(Statement::IterativeLoop(
										var.clone(),
										src,
										dst,
										Box::new(blk),
			...
			| Token::Keyword("DECLARE") => {
				...
				loop {
					...
					if let Some((Token::LParan, _)) = self.lexer.peek() {
						match self.parse_block(true, false, Parser::parse_variable_identifier) {
							| None => {
								return None;
							}
							| Some(vars) => {
								for (var_name, base_address) in vars {
									...
									if let Some(x) = base_address {
										...
										values.push(Some(VariableInitialValue::ValueOfPointer(x)));
									} else {
										values.push(None);
									}
								}
					...
					match var_type {
						...
						| Some(Type::Data) => {
							...
							if variables_added > 1 {
								parsing_error!(
									stmt_pos,
									"Can't declare multiples variables with the type DATA"
								);
							}
							...
							if values[variable_count - 1].is_some() {
								parsing_error!(
									stmt_pos,
									"Can't set an address for a DATA variable"
								);
							}
							values[variable_count - 1] =
								Some(VariableInitialValue::ReadOnlyArray(vals));
						...
						| Some(Type::Macro) => {
							...
							match self.lexer.next() {
								| Some((Token::String(content), pos)) => {
									for _ in 0..variables_added {
										...
										if values.pop().unwrap().is_some() {
											parsing_error!(
												pos,
												"A macro can't have a base address"
											);
										}
								...
								| Some((_, pos)) => {
									parsing_error!(pos, "Invalid value for a macro");
								}
								| None => {
									parsing_error!("Invalid value for a macro");
								}
							}
						}
						| Some(Type::Byte(n)) => {
						...
						match self.parse_constants() {
							...
							| Some(constants) => {
								...
								for i in 0..constants_count {
									values[i + variable_count_beginning] =
										Some(constants[i].clone());
								}
							}
						}
					}
			...
			| Token::LParan | Token::Identifier(_) => {
				...
				match self.parse_identifier_block(false) {
					...
					| None => {
						match self.parse_paranthesis_less_block(|parser| -> Option<Variable> {
							...
							let var_name = parser.lexer.next().unwrap().0.to_string();
				...
				match self.lexer.next() {
					...
					| Some((Token::Equal, _)) => {
						...
						for v in variables.into_iter().rev() {
							output_expr = Expression::VariableAssignment(v, Box::new(output_expr));
						}
					...
					| None => {
						parsing_error!(stmt_pos, "Invalid token")
					}
				}
			}
			| Token::Keyword("RETURN") => {
				...
				return Some(Statement::Return(match self.lexer.peek() {
					| Some((Token::SemiColon, _)) => {
						self.lexer.next();
						None
					}
			...
			| Token::Keyword("PROCEDURE") => {
				...
				let return_type;
				match self.lexer.peek() {
					| Some((Token::Keyword(_), _)) => match self.parse_type_from_token(1) {
						| Some(t) => {
							return_type = t;
						}
						| None => {
				...
				match self.parse_statement_block(Some(label)) {
					...
					| Some(block) => {
						return Some(Statement::Procedure(args, return_type, Box::new(block)));
					}
				}
			}
	...
	fn flatten_variable_initial_value(
		&self,
		vals: &mut Vec<i32>,
		constants: &Vec<VariableInitialValue>,
		pos: &Position,
	) -> Option<()> {
	...
	fn parse_type_from_token(&mut self, count: usize) -> Option<Type> {
		match self.lexer.next() {
			...
			| Some((Token::Keyword("DATA"), _)) => Some(Type::Data),
			| Some((Token::Keyword("LITERALLY"), _)) => Some(Type::Macro),
			| Some((_, pos)) => {
				parsing_error!(pos, "Unknown type");
			}
			| None => {
				parsing_error!("Unknown type");
			}
		}
	}
	...
	fn parse_constant(&mut self) -> Option<VariableInitialValue> {
		match self.lexer.next() {
			| Some((Token::Minus, _)) => match self.lexer.next() {
				| Some((Token::Number(x), _)) => Some(VariableInitialValue::Value(-x)),
				| Some((_, pos)) => {
			...
			| Some((Token::Number(x), _)) => Some(VariableInitialValue::Value(x)),
			| Some((Token::String(s), _)) => Some(VariableInitialValue::Array(constantify!(s))),
			| Some((_, pos)) => {
				parsing_error!(pos, "Invalid token")
			}
			| None => {
				parsing_error!("Unfinished declaration")
			}
		}
	}
			...
			match self.parse_statement(last_label) {
				| Some(Statement::EndOfStatement(x)) => {
					match (x, expected_identifier) {
						| (None, Some(_)) => {
							parsing_error!("Missing identifier")
						}
						| (Some(_), None) => {
							parsing_error!("Exceeding token")
						}
						| (None, None) => {}
