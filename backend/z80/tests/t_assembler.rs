...
macro_rules! test_ub {
    ($test_name:ident, [$inst:expr], [$( $values:expr ),+] $(, $var: ident: $vt: ty)*) => {
        ...
        fn $test_name() {
            ...
            let a = Assembler::new(iter::once(inst.clone()), false);
            let o: Vec<u8> = a.collect();
            assert!(o == [], "An undefined instruction has been counted as a correct one");
...
test_ub!(ld_bc_ix_, [LD(WordRegister(BC), WordRegister(IX))], [0xDD, 0x44, 0xDD, 0x4D]);
test_ub!(ld_bc_iy_, [LD(WordRegister(BC), WordRegister(IY))], [0xFD, 0x44, 0xFD, 0x4D]);
...
test_ub!(ld_ix_bc_, [LD(WordRegister(IX), WordRegister(BC))], [0xDD, 0x60, 0xDD, 0x69]);
test_ub!(ld_ix_de_, [LD(WordRegister(IX), WordRegister(DE))], [0xDD, 0x62, 0xDD, 0x6B]);
test_ub!(ld_ix_ix_, [LD(WordRegister(IX), WordRegister(IX))], [0xDD, 0x64, 0xDD, 0x6D]);
//test_ub!(ld_ix_iy_, [LD(WordRegister(IX), WordRegister(IY))], [0xDD, 0xE1, 0xFD, 0xE5]);
test_ub!(ld_iy_bc_, [LD(WordRegister(IY), WordRegister(BC))], [0xFD, 0x60, 0xFD, 0x69]);
test_ub!(ld_iy_de_, [LD(WordRegister(IY), WordRegister(DE))], [0xFD, 0x62, 0xFD, 0x6B]);
//test_ub!(ld_iy_ix_, [LD(WordRegister(IY), WordRegister(IX))], [0xFD, 0xE1, 0xDD, 0xE5]);
test_ub!(ld_iy_iy_, [LD(WordRegister(IY), WordRegister(IY))], [0xFD, 0x64, 0xFD, 0x6D]);
...
test_ub!(ld_de_ix_, [LD(WordRegister(DE), WordRegister(IX))], [0xDD, 0x54, 0xDD, 0x5D]);
test_ub!(ld_de_iy_, [LD(WordRegister(DE), WordRegister(IY))], [0xFD, 0x54, 0xFD, 0x5D]);
