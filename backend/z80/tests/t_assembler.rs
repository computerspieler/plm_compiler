...
macro_rules! check_output {
	($should_contain_data: expr, $assembler: expr, $output: expr, $expected: expr, $inst: expr) => {
		if (!$should_contain_data) {
			assert!(($assembler).has_error_occured(), "The assembler did not raise an error");
            assert!($output == [], "An instruction which should be undefined has been counted as a correct one");
		} else {
			assert!(!($assembler).has_error_occured(), "The assembler erronously raised an error");
			assert!($output != [], "Invalid output for {:?}: no output", $inst);
			assert!($output == $expected, "Invalid output for {:?}: got {:?}, expected {:?}", $inst, $output, $expected);
		}
	}
}

fn main_checks(
	inst: Instruction<u8, u16, i32, i8>,
	is_macro_instruction: bool,
	is_undocumented: bool,
	expected: Vec<u8>
) {
	let mut output: Vec<u8> = Vec::<u8>::with_capacity(4);
	let mut assembler = Assembler::new(iter::once(inst.clone()), false, false);
	assembler.clear_and_collect_into(&mut output);
	check_output!(!is_macro_instruction && !is_undocumented, assembler, output, expected, inst);

	let mut assembler = Assembler::new(iter::once(inst.clone()), false, true);
	assembler.clear_and_collect_into(&mut output);
	check_output!(!is_macro_instruction, assembler, output, expected, inst);

	let mut assembler = Assembler::new(iter::once(inst.clone()), true, false);
	assembler.clear_and_collect_into(&mut output);
	check_output!(!is_undocumented, assembler, output, expected, inst);

	let mut assembler = Assembler::new(iter::once(inst.clone()), true, true);
	assembler.clear_and_collect_into(&mut output);
	check_output!(true, assembler, output, expected, inst);
}
...
macro_rules! test {
    ($test_name:ident, [$inst:expr], [$( $values:expr ),+] $(, $var: ident: $vt: ty)*) => {
        ...
        fn $test_name() {
			...
			main_checks($inst, false, false, e);
        }
    };
}

macro_rules! test_with_macro {
    ($test_name:ident, [$inst:expr], [$( $values:expr ),+] $(, $var: ident: $vt: ty)*) => {
        #[test]
        fn $test_name() {
			$(
...
macro_rules! test_ub {
    ($test_name:ident, [$inst:expr], [$( $values:expr ),+] $(, $var: ident: $vt: ty)*) => {
        ...
        fn $test_name() {
            ...
            let e: Vec<u8> = vec![$($values as u8),+];
			main_checks($inst, false, true, e);
        }
    }
}

macro_rules! test_ub_with_macro {
    ($test_name:ident, [$inst:expr], [$( $values:expr ),+] $(, $var: ident: $vt: ty)*) => {
        #[test]
        fn $test_name() {
			$(
...
test_with_macro!(adc_a__ix__, [ADC(ByteRegister(A), AddressRegister(IX))], [0xDD, 0x8E, 0x00]);
test!(adc_a__ix_DIS__, [ADC(ByteRegister(A),AddressRegisterWithOffset(IX, DIS))], [0xDD, 0x8E, DIS], DIS: i8);
test_with_macro!(adc_a__iy__, [ADC(ByteRegister(A), AddressRegister(IY))], [0xFD, 0x8E, 0x00]);
...
test_ub!(
	adc_a_ixh_,
	[ADC(ByteRegister(A), UndocumentedRegister(IXH))],
	[0xDD, 0x8C]
);
test_ub!(
	adc_a_ixl_,
	[ADC(ByteRegister(A), UndocumentedRegister(IXL))],
	[0xDD, 0x8D]
);
test_ub!(
	adc_a_iyh_,
	[ADC(ByteRegister(A), UndocumentedRegister(IYH))],
	[0xFD, 0x8C]
);
test_ub!(
	adc_a_iyl_,
	[ADC(ByteRegister(A), UndocumentedRegister(IYL))],
	[0xFD, 0x8D]
);
...
test_with_macro!(add_a__ix__, [ADD(ByteRegister(A), AddressRegister(IX))], [0xDD, 0x86, 0x00]);
test!(add_a__ix_DIS__, [ADD(ByteRegister(A),AddressRegisterWithOffset(IX, DIS))], [0xDD, 0x86, DIS], DIS: i8);
test_with_macro!(add_a__iy__, [ADD(ByteRegister(A), AddressRegister(IY))], [0xFD, 0x86, 0x00]);
...
test_ub!(
	add_a_ixh_,
	[ADD(ByteRegister(A), UndocumentedRegister(IXH))],
	[0xDD, 0x84]
);
test_ub!(
	add_a_ixl_,
	[ADD(ByteRegister(A), UndocumentedRegister(IXL))],
	[0xDD, 0x85]
);
test_ub!(
	add_a_iyh_,
	[ADD(ByteRegister(A), UndocumentedRegister(IYH))],
	[0xFD, 0x84]
);
test_ub!(
	add_a_iyl_,
	[ADD(ByteRegister(A), UndocumentedRegister(IYL))],
	[0xFD, 0x85]
);
...
test_with_macro!(and__ix__, [AND(AddressRegister(IX))], [0xDD, 0xA6, 0x00]);
test!(and__ix_DIS__, [AND(AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xA6, DIS], DIS: i8);
test_with_macro!(and__iy__, [AND(AddressRegister(IY))], [0xFD, 0xA6, 0x00]);
...
test_with_macro!(bit_0__ix__, [BIT(0, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x46]);
test!(bit_0__ix_DIS__, [BIT(0,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0x46], DIS: i8);
test_with_macro!(bit_0__iy__, [BIT(0, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x46]);
...
test_with_macro!(bit_1__ix__, [BIT(1, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x4E]);
test!(bit_1__ix_DIS__, [BIT(1,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0x4E], DIS: i8);
test_with_macro!(bit_1__iy__, [BIT(1, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x4E]);
...
test_with_macro!(bit_2__ix__, [BIT(2, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x56]);
test!(bit_2__ix_DIS__, [BIT(2,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0x56], DIS: i8);
test_with_macro!(bit_2__iy__, [BIT(2, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x56]);
...
test_with_macro!(bit_3__ix__, [BIT(3, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x5E]);
test!(bit_3__ix_DIS__, [BIT(3,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0x5E], DIS: i8);
test_with_macro!(bit_3__iy__, [BIT(3, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x5E]);
...
test_with_macro!(bit_4__ix__, [BIT(4, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x66]);
test!(bit_4__ix_DIS__, [BIT(4,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0x66], DIS: i8);
test_with_macro!(bit_4__iy__, [BIT(4, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x66]);
...
test_with_macro!(bit_5__ix__, [BIT(5, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x6E]);
test!(bit_5__ix_DIS__, [BIT(5,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0x6E], DIS: i8);
test_with_macro!(bit_5__iy__, [BIT(5, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x6E]);
...
test_with_macro!(bit_6__ix__, [BIT(6, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x76]);
test!(bit_6__ix_DIS__, [BIT(6,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0x76], DIS: i8);
test_with_macro!(bit_6__iy__, [BIT(6, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x76]);
...
test_with_macro!(bit_7__ix__, [BIT(7, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x7E]);
test!(bit_7__ix_DIS__, [BIT(7,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0x7E], DIS: i8);
test_with_macro!(bit_7__iy__, [BIT(7, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x7E]);
...
test_with_macro!(cp__ix__, [CP(AddressRegister(IX))], [0xDD, 0xBE, 0x00]);
test!(cp__ix_DIS__, [CP(AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xBE, DIS], DIS: i8);
test_with_macro!(cp__iy__, [CP(AddressRegister(IY))], [0xFD, 0xBE, 0x00]);
...
test_with_macro!(dec__ix__, [DEC(AddressRegister(IX))], [0xDD, 0x35, 0x00]);
test!(dec__ix_DIS__, [DEC(AddressRegisterWithOffset(IX, DIS))], [0xDD, 0x35, DIS], DIS: i8);
test_with_macro!(dec__iy__, [DEC(AddressRegister(IY))], [0xFD, 0x35, 0x00]);
...
test_with_macro!(inc__ix__, [INC(AddressRegister(IX))], [0xDD, 0x34, 0x00]);
test_with_macro!(inc__iy__, [INC(AddressRegister(IY))], [0xFD, 0x34, 0x00]);
...
test_with_macro!(ld__hl__bc_, [LD(AddressRegister(HL), WordRegister(BC))], [0x71, 0x23, 0x70, 0x2B]);
test!(ld__hl__c_, [LD(AddressRegister(HL), ByteRegister(C))], [0x71]);
test!(ld__hl__d_, [LD(AddressRegister(HL), ByteRegister(D))], [0x72]);
test_with_macro!(ld__hl__de_, [LD(AddressRegister(HL), WordRegister(DE))], [0x73, 0x23, 0x72, 0x2B]);
...
test_with_macro!(ld__ix__a_, [LD(AddressRegister(IX), ByteRegister(A))], [0xDD, 0x77, 0x00]);
test_with_macro!(ld__ix__b_, [LD(AddressRegister(IX), ByteRegister(B))], [0xDD, 0x70, 0x00]);
test_with_macro!(ld__ix__bc_, [LD(AddressRegister(IX), WordRegister(BC))], [0xDD, 0x71, 0x00, 0xDD, 0x70, 0x01]);
test_with_macro!(ld__ix__c_, [LD(AddressRegister(IX), ByteRegister(C))], [0xDD, 0x71, 0x00]);
test_with_macro!(ld__ix__d_, [LD(AddressRegister(IX), ByteRegister(D))], [0xDD, 0x72, 0x00]);
test_with_macro!(ld__ix__de_, [LD(AddressRegister(IX), WordRegister(DE))], [0xDD, 0x73, 0x00, 0xDD, 0x72, 0x01]);
test_with_macro!(ld__ix__e_, [LD(AddressRegister(IX), ByteRegister(E))], [0xDD, 0x73, 0x00]);
test_with_macro!(ld__ix__h_, [LD(AddressRegister(IX), ByteRegister(H))], [0xDD, 0x74, 0x00]);
test_with_macro!(ld__ix__hl_, [LD(AddressRegister(IX), WordRegister(HL))], [0xDD, 0x75, 0x00, 0xDD, 0x74, 0x01]);
test_with_macro!(ld__ix__l_, [LD(AddressRegister(IX), ByteRegister(L))], [0xDD, 0x75, 0x00]);
test_with_macro!(ld__iy__a_, [LD(AddressRegister(IY), ByteRegister(A))], [0xFD, 0x77, 0x00]);
test_with_macro!(ld__iy__b_, [LD(AddressRegister(IY), ByteRegister(B))], [0xFD, 0x70, 0x00]);
test_with_macro!(ld__iy__bc_, [LD(AddressRegister(IY), WordRegister(BC))], [0xFD, 0x71, 0x00, 0xFD, 0x70, 0x01]);
test_with_macro!(ld__iy__c_, [LD(AddressRegister(IY), ByteRegister(C))], [0xFD, 0x71, 0x00]);
test_with_macro!(ld__iy__d_, [LD(AddressRegister(IY), ByteRegister(D))], [0xFD, 0x72, 0x00]);
test_with_macro!(ld__iy__de_, [LD(AddressRegister(IY), WordRegister(DE))], [0xFD, 0x73, 0x00, 0xFD, 0x72, 0x01]);
test_with_macro!(ld__iy__e_, [LD(AddressRegister(IY), ByteRegister(E))], [0xFD, 0x73, 0x00]);
test_with_macro!(ld__iy__h_, [LD(AddressRegister(IY), ByteRegister(H))], [0xFD, 0x74, 0x00]);
test_with_macro!(ld__iy__hl_, [LD(AddressRegister(IY), WordRegister(HL))], [0xFD, 0x75, 0x00, 0xFD, 0x74, 0x01]);
test_with_macro!(ld__iy__l_, [LD(AddressRegister(IY), ByteRegister(L))], [0xFD, 0x75, 0x00]);
test!(ld_a__de__, [LD(ByteRegister(A), AddressRegister(DE))], [0x1A]);
test!(ld_a__hl__, [LD(ByteRegister(A), AddressRegister(HL))], [0x7E]);
test_with_macro!(ld_a__ix__, [LD(ByteRegister(A), AddressRegister(IX))], [0xDD, 0x7E, 0x00]);
test_with_macro!(ld_a__iy__, [LD(ByteRegister(A), AddressRegister(IY))], [0xFD, 0x7E, 0x00]);
...
test_with_macro!(ld_b__ix__, [LD(ByteRegister(B), AddressRegister(IX))], [0xDD, 0x46, 0x00]);
test_with_macro!(ld_b__iy__, [LD(ByteRegister(B), AddressRegister(IY))], [0xFD, 0x46, 0x00]);
...
test_with_macro!(ld_bc__hl__, [LD(WordRegister(BC), AddressRegister(HL))], [0x4E, 0x23, 0x46, 0x2B]);
test_with_macro!(ld_bc__ix__, [LD(WordRegister(BC), AddressRegister(IX))], [0xDD, 0x4E, 0x00, 0xDD, 0x46, 0x01]);
test_with_macro!(ld_bc__iy__, [LD(WordRegister(BC), AddressRegister(IY))], [0xFD, 0x4E, 0x00, 0xFD, 0x46, 0x01]);
test_with_macro!(ld_bc_bc_, [LD(WordRegister(BC), WordRegister(BC))], [0x40, 0x49]);
test_with_macro!(ld_bc_de_, [LD(WordRegister(BC), WordRegister(DE))], [0x42, 0x4B]);
test_with_macro!(ld_bc_hl_, [LD(WordRegister(BC), WordRegister(HL))], [0x44, 0x4D]);
test_ub_with_macro!(ld_bc_ix_, [LD(WordRegister(BC), WordRegister(IX))], [0xDD, 0x44, 0xDD, 0x4D]);
test_ub_with_macro!(ld_bc_iy_, [LD(WordRegister(BC), WordRegister(IY))], [0xFD, 0x44, 0xFD, 0x4D]);
test!(ld_c__hl__, [LD(ByteRegister(C), AddressRegister(HL))], [0x4E]);
test_with_macro!(ld_c__ix__, [LD(ByteRegister(C), AddressRegister(IX))], [0xDD, 0x4E, 0x00]);
test_with_macro!(ld_c__iy__, [LD(ByteRegister(C), AddressRegister(IY))], [0xFD, 0x4E, 0x00]);
...
test_with_macro!(ld_d__ix__, [LD(ByteRegister(D), AddressRegister(IX))], [0xDD, 0x56, 0x00]);
test_with_macro!(ld_d__iy__, [LD(ByteRegister(D), AddressRegister(IY))], [0xFD, 0x56, 0x00]);
...
test_with_macro!(ld_hl_bc_, [LD(WordRegister(HL), WordRegister(BC))], [0x60, 0x69]);
test_with_macro!(ld_hl_de_, [LD(WordRegister(HL), WordRegister(DE))], [0x62, 0x6B]);
test_with_macro!(ld_hl_hl_, [LD(WordRegister(HL), WordRegister(HL))], [0x64, 0x6D]);
test!(ld_i_a_, [LD(I, ByteRegister(A))], [0xED, 0x47]);
//test_ub!(ld_hl_ix_, [LD(WordRegister(HL), WordRegister(IX))], [0xDD, 0xE5, 0xE1]);
//test_ub!(ld_hl_iy_, [LD(WordRegister(HL), WordRegister(IY))], [0xFD, 0xE5, 0xE1]);
//test_ub!(ld_ix_hl_, [LD(WordRegister(IX), WordRegister(HL))], [0xE5, 0xDD, 0xE1]);
//test_ub!(ld_iy_hl_, [LD(WordRegister(IY), WordRegister(HL))], [0xE5, 0xFD, 0xE1]);
test_ub_with_macro!(ld_ix_bc_, [LD(WordRegister(IX), WordRegister(BC))], [0xDD, 0x60, 0xDD, 0x69]);
test_ub_with_macro!(ld_ix_de_, [LD(WordRegister(IX), WordRegister(DE))], [0xDD, 0x62, 0xDD, 0x6B]);
test_ub_with_macro!(ld_ix_ix_, [LD(WordRegister(IX), WordRegister(IX))], [0xDD, 0x64, 0xDD, 0x6D]);
//test_ub!(ld_ix_iy_, [LD(WordRegister(IX), WordRegister(IY))], [0xDD, 0xE1, 0xFD, 0xE5]);
test_ub_with_macro!(ld_iy_bc_, [LD(WordRegister(IY), WordRegister(BC))], [0xFD, 0x60, 0xFD, 0x69]);
test_ub_with_macro!(ld_iy_de_, [LD(WordRegister(IY), WordRegister(DE))], [0xFD, 0x62, 0xFD, 0x6B]);
//test_ub!(ld_iy_ix_, [LD(WordRegister(IY), WordRegister(IX))], [0xFD, 0xE1, 0xDD, 0xE5]);
test_ub_with_macro!(ld_iy_iy_, [LD(WordRegister(IY), WordRegister(IY))], [0xFD, 0x64, 0xFD, 0x6D]);
test!(ld_l__hl__, [LD(ByteRegister(L), AddressRegister(HL))], [0x6E]);
test_with_macro!(ld_l__ix__, [LD(ByteRegister(L), AddressRegister(IX))], [0xDD, 0x6E, 0x00]);
test_with_macro!(ld_l__iy__, [LD(ByteRegister(L), AddressRegister(IY))], [0xFD, 0x6E, 0x00]);
...
test_ub!(
	ld_a_ixh_,
	[LD(ByteRegister(A), UndocumentedRegister(IXH))],
	[0xDD, 0x7C]
);
test_ub!(
	ld_a_ixl_,
	[LD(ByteRegister(A), UndocumentedRegister(IXL))],
	[0xDD, 0x7D]
);
test_ub!(
	ld_a_iyh_,
	[LD(ByteRegister(A), UndocumentedRegister(IYH))],
	[0xFD, 0x7C]
);
test_ub!(
	ld_a_iyl_,
	[LD(ByteRegister(A), UndocumentedRegister(IYL))],
	[0xFD, 0x7D]
);
test_ub!(
	ld_b_ixh_,
	[LD(ByteRegister(B), UndocumentedRegister(IXH))],
	[0xDD, 0x44]
);
test_ub!(
	ld_b_ixl_,
	[LD(ByteRegister(B), UndocumentedRegister(IXL))],
	[0xDD, 0x45]
);
test_ub!(
	ld_b_iyh_,
	[LD(ByteRegister(B), UndocumentedRegister(IYH))],
	[0xFD, 0x44]
);
test_ub!(
	ld_b_iyl_,
	[LD(ByteRegister(B), UndocumentedRegister(IYL))],
	[0xFD, 0x45]
);
test_ub!(
	ld_c_ixh_,
	[LD(ByteRegister(C), UndocumentedRegister(IXH))],
	[0xDD, 0x4C]
);
test_ub!(
	ld_c_ixl_,
	[LD(ByteRegister(C), UndocumentedRegister(IXL))],
	[0xDD, 0x4D]
);
test_ub!(
	ld_c_iyh_,
	[LD(ByteRegister(C), UndocumentedRegister(IYH))],
	[0xFD, 0x4C]
);
test_ub!(
	ld_c_iyl_,
	[LD(ByteRegister(C), UndocumentedRegister(IYL))],
	[0xFD, 0x4D]
);
...
test_ub!(
	ld_d_ixh_,
	[LD(ByteRegister(D), UndocumentedRegister(IXH))],
	[0xDD, 0x54]
);
test_ub!(
	ld_d_ixl_,
	[LD(ByteRegister(D), UndocumentedRegister(IXL))],
	[0xDD, 0x55]
);
test_ub!(
	ld_d_iyh_,
	[LD(ByteRegister(D), UndocumentedRegister(IYH))],
	[0xFD, 0x54]
);
test_ub!(
	ld_d_iyl_,
	[LD(ByteRegister(D), UndocumentedRegister(IYL))],
	[0xFD, 0x55]
);
test!(ld_d_l_, [LD(ByteRegister(D), ByteRegister(L))], [0x55]);
test!(ld_e__hl__, [LD(ByteRegister(E), AddressRegister(HL))], [0x5E]);
test_with_macro!(ld_e__ix__, [LD(ByteRegister(E), AddressRegister(IX))], [0xDD, 0x5E, 0x00]);
...
test_ub!(
	ld_e_ixh_,
	[LD(ByteRegister(E), UndocumentedRegister(IXH))],
	[0xDD, 0x5C]
);
test_ub!(
	ld_e_ixl_,
	[LD(ByteRegister(E), UndocumentedRegister(IXL))],
	[0xDD, 0x5D]
);
test_ub!(
	ld_e_iyh_,
	[LD(ByteRegister(E), UndocumentedRegister(IYH))],
	[0xFD, 0x5C]
);
test_ub!(
	ld_e_iyl_,
	[LD(ByteRegister(E), UndocumentedRegister(IYL))],
	[0xFD, 0x5D]
);
test!(ld_e_l_, [LD(ByteRegister(E), ByteRegister(L))], [0x5D]);
test!(ld_h__hl__, [LD(ByteRegister(H), AddressRegister(HL))], [0x66]);
test_with_macro!(ld_h__ix__, [LD(ByteRegister(H), AddressRegister(IX))], [0xDD, 0x66, 0x00]);
test_with_macro!(ld_h__iy__, [LD(ByteRegister(H), AddressRegister(IY))], [0xFD, 0x66, 0x00]);
test_with_macro!(ld_hl__ix__, [LD(WordRegister(HL), AddressRegister(IX))], [0xDD, 0x6E, 0x00, 0xDD, 0x66, 0x01]);
test_with_macro!(ld_hl__iy__, [LD(WordRegister(HL), AddressRegister(IY))], [0xFD, 0x6E, 0x00, 0xFD, 0x66, 0x01]);
test_ub!(
	ld_ixh_a_,
...
test_ub!(
	ld_ixh_b_,
	[LD(UndocumentedRegister(IXH), ByteRegister(B))],
	[0xDD, 0x60]
);
test_ub!(
	ld_ixh_c_,
	[LD(UndocumentedRegister(IXH), ByteRegister(C))],
	[0xDD, 0x61]
);
test_ub!(
	ld_ixh_d_,
	[LD(UndocumentedRegister(IXH), ByteRegister(D))],
	[0xDD, 0x62]
);
test_ub!(
	ld_ixh_e_,
	[LD(UndocumentedRegister(IXH), ByteRegister(E))],
	[0xDD, 0x63]
);
test_ub!(
	ld_ixh_ixh_,
	[LD(UndocumentedRegister(IXH), UndocumentedRegister(IXH))],
	[0xDD, 0x64]
);
test_ub!(
	ld_ixh_ixl_,
	[LD(UndocumentedRegister(IXH), UndocumentedRegister(IXL))],
	[0xDD, 0x65]
);
test_ub!(
	ld_ixl_a_,
	[LD(UndocumentedRegister(IXL), ByteRegister(A))],
	[0xDD, 0x6F]
);
test_ub!(
	ld_ixl_b_,
	[LD(UndocumentedRegister(IXL), ByteRegister(B))],
	[0xDD, 0x68]
);
test_ub!(
	ld_ixl_c_,
	[LD(UndocumentedRegister(IXL), ByteRegister(C))],
	[0xDD, 0x69]
);
test_ub!(
	ld_ixl_d_,
	[LD(UndocumentedRegister(IXL), ByteRegister(D))],
	[0xDD, 0x6A]
);
test_ub!(
	ld_ixl_e_,
	[LD(UndocumentedRegister(IXL), ByteRegister(E))],
	[0xDD, 0x6B]
);
test_ub!(
	ld_ixl_ixh_,
	[LD(UndocumentedRegister(IXL), UndocumentedRegister(IXH))],
	[0xDD, 0x6C]
);
test_ub!(
	ld_ixl_ixl_,
	[LD(UndocumentedRegister(IXL), UndocumentedRegister(IXL))],
	[0xDD, 0x6D]
);
test_ub!(
	ld_iyh_a_,
	[LD(UndocumentedRegister(IYH), ByteRegister(A))],
	[0xFD, 0x67]
);
test_ub!(
	ld_iyh_b_,
	[LD(UndocumentedRegister(IYH), ByteRegister(B))],
	[0xFD, 0x60]
);
test_ub!(
	ld_iyh_c_,
	[LD(UndocumentedRegister(IYH), ByteRegister(C))],
	[0xFD, 0x61]
);
test_ub!(
	ld_iyh_d_,
	[LD(UndocumentedRegister(IYH), ByteRegister(D))],
	[0xFD, 0x62]
);
test_ub!(
	ld_iyh_e_,
	[LD(UndocumentedRegister(IYH), ByteRegister(E))],
	[0xFD, 0x63]
);
test_ub!(
	ld_iyh_iyh_,
	[LD(UndocumentedRegister(IYH), UndocumentedRegister(IYH))],
	[0xFD, 0x64]
);
test_ub!(
	ld_iyh_iyl_,
	[LD(UndocumentedRegister(IYH), UndocumentedRegister(IYL))],
	[0xFD, 0x65]
);
test_ub!(
	ld_iyl_a_,
	[LD(UndocumentedRegister(IYL), ByteRegister(A))],
	[0xFD, 0x6F]
);
test_ub!(
	ld_iyl_b_,
	[LD(UndocumentedRegister(IYL), ByteRegister(B))],
	[0xFD, 0x68]
);
test_ub!(
	ld_iyl_c_,
	[LD(UndocumentedRegister(IYL), ByteRegister(C))],
	[0xFD, 0x69]
);
test_ub!(
	ld_iyl_d_,
	[LD(UndocumentedRegister(IYL), ByteRegister(D))],
	[0xFD, 0x6A]
);
test_ub!(
	ld_iyl_e_,
	[LD(UndocumentedRegister(IYL), ByteRegister(E))],
	[0xFD, 0x6B]
);
test_ub!(
	ld_iyl_iyh_,
	[LD(UndocumentedRegister(IYL), UndocumentedRegister(IYH))],
	[0xFD, 0x6C]
);
test_ub!(
	ld_iyl_iyl_,
	[LD(UndocumentedRegister(IYL), UndocumentedRegister(IYL))],
	[0xFD, 0x6D]
);
...
test_with_macro!(or__ix__, [OR(AddressRegister(IX))], [0xDD, 0xB6, 0x00]);
test_with_macro!(or__iy__, [OR(AddressRegister(IY))], [0xFD, 0xB6, 0x00]);
...
test_with_macro!(res_0__ix__, [RES(0, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x86]);
...
test_with_macro!(res_0__iy__, [RES(0, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x86]);
...
test_with_macro!(res_1__ix__, [RES(1, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x8E]);
test!(res_1__ix_DIS__, [RES(1,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0x8E], DIS: i8);
test_with_macro!(res_1__iy__, [RES(1, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x8E]);
...
test_with_macro!(res_2__ix__, [RES(2, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x96]);
test!(res_2__ix_DIS__, [RES(2,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0x96], DIS: i8);
test_with_macro!(res_2__iy__, [RES(2, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x96]);
...
test_with_macro!(res_3__ix__, [RES(3, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x9E]);
test!(res_3__ix_DIS__, [RES(3,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0x9E], DIS: i8);
test_with_macro!(res_3__iy__, [RES(3, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x9E]);
...
test_with_macro!(res_4__ix__, [RES(4, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0xA6]);
test!(res_4__ix_DIS__, [RES(4,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0xA6], DIS: i8);
test_with_macro!(res_4__iy__, [RES(4, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0xA6]);
...
test_with_macro!(res_5__ix__, [RES(5, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0xAE]);
test!(res_5__ix_DIS__, [RES(5,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0xAE], DIS: i8);
test_with_macro!(res_5__iy__, [RES(5, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0xAE]);
...
test_with_macro!(res_6__ix__, [RES(6, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0xB6]);
test!(res_6__ix_DIS__, [RES(6,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0xB6], DIS: i8);
test_with_macro!(res_6__iy__, [RES(6, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0xB6]);
...
test_with_macro!(res_7__ix__, [RES(7, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0xBE]);
test!(res_7__ix_DIS__, [RES(7,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0xBE], DIS: i8);
test_with_macro!(res_7__iy__, [RES(7, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0xBE]);
...
test_with_macro!(rl__ix__, [RL(AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x16]);
test_with_macro!(rl__iy__, [RL(AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x16]);
...
test_with_macro!(rl_bc_, [RL(WordRegister(BC))], [0xCB, 0x11, 0xCB, 0x10]);
test!(rl_c_, [RL(ByteRegister(C))], [0xCB, 0x11]);
test!(rl_d_, [RL(ByteRegister(D))], [0xCB, 0x12]);
test_with_macro!(rl_de_, [RL(WordRegister(DE))], [0xCB, 0x13, 0xCB, 0x12]);
test!(rl_e_, [RL(ByteRegister(E))], [0xCB, 0x13]);
test!(rl_h_, [RL(ByteRegister(H))], [0xCB, 0x14]);
test_with_macro!(rl_hl_, [RL(WordRegister(HL))], [0xCB, 0x15, 0xCB, 0x14]);
test!(rl_l_, [RL(ByteRegister(L))], [0xCB, 0x15]);
test!(rla, [RLA], [0x17]);
test!(rlc__hl__, [RLC(AddressRegister(HL))], [0xCB, 0x06]);
test_with_macro!(rlc__ix__, [RLC(AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x06]);
test!(rlc__ix_DIS__, [RLC(AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0x06], DIS: i8);
test_with_macro!(rlc__iy__, [RLC(AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x06]);
...
test_with_macro!(rr__ix__, [RR(AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x1E]);
test!(rr__ix_DIS__, [RR(AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0x1E], DIS: i8);
test_with_macro!(rr__iy__, [RR(AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x1E]);
...
test_with_macro!(rr_bc_, [RR(WordRegister(BC))], [0xCB, 0x18, 0xCB, 0x19]);
test!(rr_c_, [RR(ByteRegister(C))], [0xCB, 0x19]);
test!(rr_d_, [RR(ByteRegister(D))], [0xCB, 0x1A]);
test_with_macro!(rr_de_, [RR(WordRegister(DE))], [0xCB, 0x1A, 0xCB, 0x1B]);
test!(rr_e_, [RR(ByteRegister(E))], [0xCB, 0x1B]);
test!(rr_h_, [RR(ByteRegister(H))], [0xCB, 0x1C]);
test_with_macro!(rr_hl_, [RR(WordRegister(HL))], [0xCB, 0x1C, 0xCB, 0x1D]);
test!(rr_l_, [RR(ByteRegister(L))], [0xCB, 0x1D]);
test!(rra, [RRA], [0x1F]);
test!(rrc__hl__, [RRC(AddressRegister(HL))], [0xCB, 0x0E]);
test_with_macro!(rrc__ix__, [RRC(AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x0E]);
test!(rrc__ix_DIS__, [RRC(AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0x0E], DIS: i8);
test_with_macro!(rrc__iy__, [RRC(AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x0E]);
...
test_with_macro!(sbc_a__ix__, [SBC(ByteRegister(A), AddressRegister(IX))], [0xDD, 0x9E, 0x00]);
test!(sbc_a__ix_DIS__, [SBC(ByteRegister(A),AddressRegisterWithOffset(IX, DIS))], [0xDD, 0x9E, DIS], DIS: i8);
test_with_macro!(sbc_a__iy__, [SBC(ByteRegister(A), AddressRegister(IY))], [0xFD, 0x9E, 0x00]);
...
test_ub!(
	sbc_a_ixh_,
	[SBC(ByteRegister(A), UndocumentedRegister(IXH))],
	[0xDD, 0x9C]
);
test_ub!(
	sbc_a_ixl_,
	[SBC(ByteRegister(A), UndocumentedRegister(IXL))],
	[0xDD, 0x9D]
);
test_ub!(
	sbc_a_iyh_,
	[SBC(ByteRegister(A), UndocumentedRegister(IYH))],
	[0xFD, 0x9C]
);
test_ub!(
	sbc_a_iyl_,
	[SBC(ByteRegister(A), UndocumentedRegister(IYL))],
	[0xFD, 0x9D]
);
...
test_with_macro!(set_0__ix__, [SET(0, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0xC6]);
test!(set_0__ix_DIS__, [SET(0,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0xC6], DIS: i8);
test_with_macro!(set_0__iy__, [SET(0, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0xC6]);
...
test_with_macro!(set_1__ix__, [SET(1, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0xCE]);
test!(set_1__ix_DIS__, [SET(1,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0xCE], DIS: i8);
test_with_macro!(set_1__iy__, [SET(1, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0xCE]);
...
test_with_macro!(set_2__ix__, [SET(2, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0xD6]);
test!(set_2__ix_DIS__, [SET(2,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0xD6], DIS: i8);
test_with_macro!(set_2__iy__, [SET(2, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0xD6]);
...
test_with_macro!(set_3__ix__, [SET(3, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0xDE]);
test!(set_3__ix_DIS__, [SET(3,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0xDE], DIS: i8);
test_with_macro!(set_3__iy__, [SET(3, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0xDE]);
...
test_with_macro!(set_4__ix__, [SET(4, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0xE6]);
test!(set_4__ix_DIS__, [SET(4,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0xE6], DIS: i8);
test_with_macro!(set_4__iy__, [SET(4, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0xE6]);
...
test_with_macro!(set_5__ix__, [SET(5, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0xEE]);
test!(set_5__ix_DIS__, [SET(5,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0xEE], DIS: i8);
test_with_macro!(set_5__iy__, [SET(5, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0xEE]);
...
test_with_macro!(set_6__ix__, [SET(6, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0xF6]);
test!(set_6__ix_DIS__, [SET(6,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0xF6], DIS: i8);
test_with_macro!(set_6__iy__, [SET(6, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0xF6]);
...
test_with_macro!(set_7__ix__, [SET(7, AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0xFE]);
test!(set_7__ix_DIS__, [SET(7,AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0xFE], DIS: i8);
test_with_macro!(set_7__iy__, [SET(7, AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0xFE]);
...
test_with_macro!(srl_bc_, [SRL(WordRegister(BC))], [0xCB, 0x38, 0xCB, 0x19]);
test_with_macro!(srl_de_, [SRL(WordRegister(DE))], [0xCB, 0x3A, 0xCB, 0x1B]);
test_with_macro!(srl_hl_, [SRL(WordRegister(HL))], [0xCB, 0x3C, 0xCB, 0x1D]);
test!(srl__hl__, [SRL(AddressRegister(HL))], [0xCB, 0x3E]);
test_with_macro!(srl__ix__, [SRL(AddressRegister(IX))], [0xDD, 0xCB, 0x00, 0x3E]);
test!(srl__ix_DIS__, [SRL(AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xCB, DIS, 0x3E], DIS: i8);
test_with_macro!(srl__iy__, [SRL(AddressRegister(IY))], [0xFD, 0xCB, 0x00, 0x3E]);
...
test_with_macro!(sub__ix__, [SUB(AddressRegister(IX))], [0xDD, 0x96, 0x00]);
test!(sub__ix_DIS__, [SUB(AddressRegisterWithOffset(IX, DIS))], [0xDD, 0x96, DIS], DIS: i8);
test_with_macro!(sub__iy__, [SUB(AddressRegister(IY))], [0xFD, 0x96, 0x00]);
...
test_with_macro!(xor__ix__, [XOR(AddressRegister(IX))], [0xDD, 0xAE, 0x00]);
test!(xor__ix_DIS__, [XOR(AddressRegisterWithOffset(IX, DIS))], [0xDD, 0xAE, DIS], DIS: i8);
test_with_macro!(xor__iy__, [XOR(AddressRegister(IY))], [0xFD, 0xAE, 0x00]);
...
test_with_macro!(ld__ix__N_, [LD(AddressRegister(IX),Constant(n as i32))], [0xDD, 0x36, 0x00, n], n: u8);
test_with_macro!(ld__iy__N_, [LD(AddressRegister(IY),Constant(n as i32))], [0xFD, 0x36, 0x00, n], n: u8);
...
test_with_macro!(ld__ix_DIS__bc_, [LD(AddressRegisterWithOffset(IX, DIS),WordRegister(BC))], [0xDD, 0x71, DIS, 0xDD, 0x70, DIS+1], DIS: i8);
...
test_with_macro!(ld__ix_DIS__de_, [LD(AddressRegisterWithOffset(IX, DIS),WordRegister(DE))], [0xDD, 0x73, DIS, 0xDD, 0x72, DIS+1], DIS: i8);
...
test_with_macro!(ld__ix_DIS__hl_, [LD(AddressRegisterWithOffset(IX, DIS),WordRegister(HL))], [0xDD, 0x75, DIS, 0xDD, 0x74, DIS+1], DIS: i8);
...
test_with_macro!(ld__iy_DIS__bc_, [LD(AddressRegisterWithOffset(IY, DIS),WordRegister(BC))], [0xFD, 0x71, DIS, 0xFD, 0x70, DIS+1], DIS: i8);
...
test_with_macro!(ld__iy_DIS__de_, [LD(AddressRegisterWithOffset(IY, DIS),WordRegister(DE))], [0xFD, 0x73, DIS, 0xFD, 0x72, DIS+1], DIS: i8);
...
test_with_macro!(ld__iy_DIS__hl_, [LD(AddressRegisterWithOffset(IY, DIS),WordRegister(HL))], [0xFD, 0x75, DIS, 0xFD, 0x74, DIS+1], DIS: i8);
...
test_with_macro!(ld_bc__ix_DIS__, [LD(WordRegister(BC),AddressRegisterWithOffset(IX, DIS))], [0xDD, 0x4E, DIS, 0xDD, 0x46, DIS+1], DIS: i8);
test_with_macro!(ld_bc__iy_DIS__, [LD(WordRegister(BC),AddressRegisterWithOffset(IY, DIS))], [0xFD, 0x4E, DIS, 0xFD, 0x46, DIS+1], DIS: i8);
...
test_with_macro!(ld_de__hl__, [LD(WordRegister(DE), AddressRegister(HL))], [0x5E, 0x23, 0x56, 0x2B]);
test_with_macro!(ld_de__ix__, [LD(WordRegister(DE), AddressRegister(IX))], [0xDD, 0x5E, 0x00, 0xDD, 0x56, 0x01]);
test_with_macro!(ld_de__ix_DIS__, [LD(WordRegister(DE),AddressRegisterWithOffset(IX, DIS))], [0xDD, 0x5E, DIS, 0xDD, 0x56, DIS+1], DIS: i8);
test_with_macro!(ld_de__iy__, [LD(WordRegister(DE), AddressRegister(IY))], [0xFD, 0x5E, 0x00, 0xFD, 0x56, 0x01]);
test_with_macro!(ld_de__iy_DIS__, [LD(WordRegister(DE),AddressRegisterWithOffset(IY, DIS))], [0xFD, 0x5E, DIS, 0xFD, 0x56, DIS+1], DIS: i8);
test!(ld_de_NN_, [LD(WordRegister(DE),Constant(NN as i32))], [0x11, (NN & 0xFF), ((NN >> 8) & 0xFF)], NN: u16);
test_with_macro!(ld_de_bc_, [LD(WordRegister(DE), WordRegister(BC))], [0x50, 0x59]);
test_with_macro!(ld_de_de_, [LD(WordRegister(DE), WordRegister(DE))], [0x52, 0x5B]);
test_with_macro!(ld_de_hl_, [LD(WordRegister(DE), WordRegister(HL))], [0x54, 0x5D]);
test_ub_with_macro!(ld_de_ix_, [LD(WordRegister(DE), WordRegister(IX))], [0xDD, 0x54, 0xDD, 0x5D]);
test_ub_with_macro!(ld_de_iy_, [LD(WordRegister(DE), WordRegister(IY))], [0xFD, 0x54, 0xFD, 0x5D]);
test!(ld_e__ix_DIS__, [LD(ByteRegister(E),AddressRegisterWithOffset(IX, DIS))], [0xDD, 0x5E, DIS], DIS: i8);
test_with_macro!(ld_e__iy__, [LD(ByteRegister(E), AddressRegister(IY))], [0xFD, 0x5E, 0x00]);
...
test_with_macro!(ld_hl__ix_DIS__, [LD(WordRegister(HL),AddressRegisterWithOffset(IX, DIS))], [0xDD, 0x6E, DIS, 0xDD, 0x66, DIS+1], DIS: i8);
test_with_macro!(ld_hl__iy_DIS__, [LD(WordRegister(HL),AddressRegisterWithOffset(IY, DIS))], [0xFD, 0x6E, DIS, 0xFD, 0x66, DIS+1], DIS: i8);
