...
impl<InputType: Iterator<Item = Instruction>> Assembler<InputType> {
    ...
    fn convert_instruction(&mut self, inst: Instruction) -> bool {
        ...
        match inst {
            ...
            ADD(WordRegister(IX), WordRegister(pp)) => b![0xDD, 0x09 | self.get_pp_value(pp) << 4],
            ADD(WordRegister(IY), WordRegister(rr)) => b![0xFD, 0x09 | self.get_rr_value(rr) << 4],

            ADC(WordRegister(HL), WordRegister(ss)) => b![0xED, 0x4A | self.get_ss_value(ss) << 4],

            SBC(WordRegister(HL), WordRegister(ss)) => b![0xED, 0x42 | self.get_ss_value(ss) << 4],
